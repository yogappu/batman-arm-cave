...
#define GPIO_ODR_ODR10   (1 << 10)    // ODR10 is the 10th bit in the ODR register
...
void configure_gpio(void) {
    ...
    // Configure PC10
    GPIOC_CRH &= ~((0x0F) << 8);  // Clear all 4 bits for PC10 (MODE10[1:0] and CNF10[1:0])
    GPIOC_CRH |= (0x03 << 8);     // Set MODE10 to output 50 MHz (binary 11)
    GPIOC_CRH &= ~(0x03 << 10);   // Set CNF10 to push-pull (00)

    // Set initial state
    GPIOC_ODR |= GPIO_ODR_ODR8;  // Set PC8 high
    GPIOC_ODR |= GPIO_ODR_ODR9;  // Set PC9 high
    //GPIOC_ODR |= GPIO_ODR_ODR10; // Set PC10 high
